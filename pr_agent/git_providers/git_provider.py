...
class GitProvider(ABC):
    ...
    def get_user_description(self) -> str:
        ...
        for header in possible_headers: # try to clip at the next header after start_position
            if header != user_description_header:
                pos = description_lowercase.find(header, start_position)  # search AFTER start_position
                if pos != -1:
                    end_position = min(end_position, pos)
    ...
    def _possible_headers(self):
        return ("### **user description**", "### **pr type**", "### **pr description**", "### **pr labels**", "### **type**", "### **description**",
                "### **labels**", "### 🤖 generated by pr agent", "### **jira test cases**", "### **test cases**")
